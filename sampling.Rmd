...
```{r kdetrain, eval=TRUE, fig.width=8,fig.height=4,fig.cap="\\label{fig:densf}a)Distribution of all GSV training images; b) only training pairs that are positive for change; c) only pairs that are negative for change. "}
# makeplot
...
```{r figs, fig.width=7,fig.height=6,fig.cap="\\label{fig:figs}Distribution of GSV dates within accessed GSV imagery"}
hist(gsvdates$OLDEST,xlab="Date",ylab="Frequency",main="")
```

The \ref{fig:figs} indicates the approximate 2-year update frequency for GSV imagery.
...
```{r plot_comparison_model_permits, eval=TRUE, fig.width=8,fig.height=4,fig.cap="\\label{fig:plot_comparison_model_permits}Distribution of GSV dates within accessed GSV imagery"}
...
plot(
  nbrhoods,
  add = TRUE,
  border = rgb(144 / 255, 144 / 255, 144 / 255, 1),
  lwd = 0.5
)
...
plot(
  kdepredict,
  legend.only = TRUE,
  breaks = pretty(r.range),
  col = my.palette,
  axes = FALSE,
  legend.width = 1,
  legend.shrink = 0.35,
  horizontal = FALSE,zlim=pretty(r.range),
...
```
The pattern of building permits and SCNN-FC-8 predictions are very similar in Figure \ref{fig:plot_comparison_model_permits}.

```{r, savedensfplot, eval=TRUE, echo=FALSE}
# makeplot
...
```{r comparemodelpermit, echo=FALSE,eval=FALSE}
# Calculate KDE on training locations & convert to raster obj
