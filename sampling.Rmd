...
```{r trainingdata,eval=FALSE, message=FALSE}
...
```{r kdetrain, eval=TRUE, fig.width=8,fig.height=4,fig.cap="\\label{fig:densf}Distribution of GSV dates within accessed GSV imagery"}
# makeplot
...
```{r comparemodelpermit, echo=TRUE,eval=FALSE}
...
##Results: KDE of permits and predictions
```{r aname}
...
```{r densf, eval=TRUE, fig.width=8,fig.height=4,fig.cap="\\label{fig:densf}Distribution of GSV dates within accessed GSV imagery"}
# makeplot
...
```{r, saveplot, eval=TRUE, echo=FALSE}
# makeplot
windows(width=8,height=4,xpinch=300,ypinch=300)
par(mfrow=c(1,2),mai=c(0,0,0,0),bty="n")
plot(kdepredict, breaks=cint$brks,col = my.palette,axes=FALSE,legend=FALSE)
...
The pattern of building permits and SCNN-FC-8 predictions are very similar in Figure \ref{fig:densf}.
